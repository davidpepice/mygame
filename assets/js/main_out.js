(function(wHandle, wjQuery) {
    var CONNECTION_URL = "customserver.glitch.me:8080", // Default Connection
        ...
        wHandle.onkeydown = function(event) {
            switch (event.keyCode) {
                ...
                case 32: // space
                    ...
                    break;
                case 87: // W
                //let wmacro = setTimeout(() =>{
                    if ((!wPressed) && (!isTyping)) {
                        //sendMouseMove(); 
                         sendUint8(21);
                         if (!wMacro) rPressed = true;
                         wPressed = true; 
                    } 
                //}, 1);
                    break;
    ...
    function drawLeaderBoard() {
        ...
        if (drawTeam || 0 != leaderBoard.length)
            if (drawTeam || showName) {
                ...
                ctx.globalAlpha = .4;
                ctx.fillStyle = "#000000";
              //ctx.fillRect(10, 38, 200, boardLength);
                ctx.globalAlpha = 1;
                ...
                var b, l;
                if (!drawTeam) {
                    for (ctx.font = "22px Ubuntu", b = 0, l = leaderBoard.length; b < l; ++b) {
                         console.log( );
                        c = leaderBoard[b].name || "An unnamed cell";
                        //$("body").append('<div id="LeaderName" style="position:absolute; width:130px; top: 147px; right:10px;   color:#ffffff;  padding:5px;border-radius:5px; cursor:pointer;"><span  ></span>'+ me +'</div>');
                        if (!showName) {
                            (c = "An unnamed cell");
                        }
                        var me = -1 != nodesOnScreen.indexOf(leaderBoard[b].id);
                        if (me) playerCells[0].name && (c = playerCells[0].name);
                        me ? ctx.fillStyle = "#eff176" : ctx.fillStyle = "#fff";

                        if (!showDarkTheme) {
                            ctx.fillStyle = showDarkTheme ? "#fff" : "#000";
                        }
                ...
                if (!drawTeam) {
                    for (ctx.font = "22px Ubuntu", b = 10, l = leaderBoard.length; b < l; ++b) {
                         console.log( );
                        c = leaderBoard[b].name || "An unnamed cell";
                        //$("body").append('<div id="LeaderName" style="position:absolute; width:130px; top: 147px; right:10px;   color:#ffffff;  padding:5px;border-radius:5px; cursor:pointer;"><span  ></span>'+ me +'</div>');
                        if (showName) {
                        ...
                        var me = -1 != nodesOnScreen.indexOf(leaderBoard[b].id);
                        if (me) playerCells[9].name && (c = playerCells[9].name);
                        me ? ctx.fillStyle = "#eff176" : ctx.fillStyle = "#000";
                        if (!noRanking) c = b + 0 + ". " + c;
                         
                        var start = (ctx.measureText(c).width > 200) ? 2 : 100 - ctx.measureText(c).width * 0.5;
                        ctx.fillText(c, start, 90 + 24);
        ...
        drawOneCell: function(ctx) {
            if (this.shouldRender()) {
                ...
                if (c) {
                    ...
                    //Draw skin
                    //ctx.drawImage(skinss, this.x - bigPointSize, this.y - bigPointSize, 2 * bigPointSize, 2 * bigPointSize);
                    ctx.drawImage(c, this.x - bigPointSize, this.y - bigPointSize, 2 * bigPointSize, 2 * bigPointSize);
                    ctx.restore();
                }
                if ((showColor || 10 < this.size) && !b) {
                ...
                if (0 != this.id) {
                    var x = ~~this.x,
                        y = ~~this.y,
                        nz = this.getNameSize(),
                        ratio = Math.ceil(10 * viewZoom) * 0.4,
                        ratD = 1 / ratio;
